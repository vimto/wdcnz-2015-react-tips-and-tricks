...
The one way data flow makes it simpler to reason about your application, and having messages flow through single points such as the dispatcher are useful for some cross cutting concerns.

It's working pretty well for us, but has been the source of some confusion.When we started down the path of using React, there weren't really any libraries out there, so we rolled our own given how small the pattern is.
...
We currently use a library called Fluxxor, which stays true to the original intent of the pattern, whilst offering some nice conveniances.
...
We're currently evaluating it for a companion app to Atmomic to let users preview thei designs, but also more interestingly as a way to let users design native UIs in the browser.
